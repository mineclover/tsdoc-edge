...
for (let i = 0; i < Math.min(result.comments.length, 3); i++) {
  ...
  console.log(
    `Summary text: ${comment.docComment.summarySection.nodes.map((n) => (n as any).text || '').join('')}`
  );
  console.log(
    `Modifier tags: ${comment.docComment.modifierTagSet.nodes.map((n) => n.tagName).join(', ')}`
  );
