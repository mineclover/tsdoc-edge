# TSDoc Edge - Core Features

> 핵심 기능 정의 문서 (Core API 기반)

## 1. Core Workflow

### TSDocEdge
Main entry point for TSDoc Edge
- 참조: `src/index.ts`

### TSDocParser
Parser for extracting and parsing TSDoc comments from TypeScript source files
- 참조: `src/parser/TSDocParser.ts`

### ParseResult
Result of parsing a source file
- 참조: `src/types/core/parse.ts`

### isEmptyOrWhitespace
Utility functions for TSDoc Edge
- 참조: `src/utils/index.ts`

### isEmptyOrWhitespace
Check if a string is empty or whitespace only
- 참조: `src/utils/index.ts`

### normalizeLineEndings
Normalize line endings to LF
- 참조: `src/utils/index.ts`

### getFileExtension
Extract file extension from path
- 참조: `src/utils/index.ts`

### isTypeScriptOrJavaScript
Check if file is TypeScript or JavaScript
- 참조: `src/utils/index.ts`

---

## 2. Analysis

### CodeHealthChecker
Checks overall code health and generates improvement suggestions
- 참조: `src/analyzer/CodeHealthChecker.ts`

### DataFlowAnalyzer
Analyzes data transformation flows and DTO patterns
- 참조: `src/analyzer/DataFlowAnalyzer.ts`

### DocumentationAnalyzer
Analyzes documentation quality of TypeScript symbols
- 참조: `src/analyzer/DocumentationAnalyzer.ts`

### DomainStructureAnalyzer
Analyzes domain structure for domain-driven development
- 참조: `src/analyzer/DomainStructureAnalyzer.ts`

### ImportanceClassifier
Classifies symbols by their importance level

Critical: Public API, exported symbols, symbols with contracts Important: Structural types, highly connected symbols Normal: Private utilities, helpers
- 참조: `src/analyzer/ImportanceClassifier.ts`

### InterfaceAnalyzer
Analyzes TypeScript interfaces for domain-driven development
- 참조: `src/analyzer/InterfaceAnalyzer.ts`

### hasExportModifier
Check if node has export modifier
- 참조: `src/analyzer/InterfaceAnalyzer.ts`

### InterfaceDependencyMapper
Maps and analyzes dependencies between interfaces
- 참조: `src/analyzer/InterfaceDependencyMapper.ts`

### StatsComparator
Compares statistics between snapshots and detects issues
- 참조: `src/analyzer/StatsComparator.ts`

### StatsHistoryManager
Manages statistics history persistence
- 참조: `src/analyzer/StatsHistoryManager.ts`

### TestCoverageAnalyzer
Analyzes test coverage by finding test files
- 참조: `src/analyzer/TestCoverageAnalyzer.ts`

### TrackableStatsCollector
Collects statistics that can be tracked over time
- 참조: `src/analyzer/TrackableStatsCollector.ts`

---

## 3. Symbol Graph

### TraversalOptions
Traversal options
- 참조: `src/graph/DepthTraverser.ts`

### TraversalResult
Result of depth traversal
- 참조: `src/graph/DepthTraverser.ts`

### DepthTraverser
Traverses symbol graph by depth levels using BFS
- 참조: `src/graph/DepthTraverser.ts`

### SymbolGraphBuilder
Builds and maintains a graph of all symbols and their relationships
- 참조: `src/graph/SymbolGraphBuilder.ts`

### SymbolSearchEngine
Search engine for finding symbols based on various criteria
- 참조: `src/graph/SymbolSearchEngine.ts`

### Symbol
Represents a symbol in the codebase
- 참조: `src/types/graph/graph.ts`

### SymbolGraph
Symbol graph representing the entire codebase structure
- 참조: `src/types/graph/graph.ts`

### ConnectivityAnalysis
Connectivity analysis result
- 참조: `src/types/graph/graph.ts`

### SymbolQuery
Query interface for symbol search
- 참조: `src/types/graph/graph.ts`

### SymbolQueryResult
Result of a symbol search query
- 참조: `src/types/graph/graph.ts`

### DetailedValidationIssue
Detailed validation issue for a specific symbol
- 참조: `src/types/graph/graph.ts`

### DetailedValidationReport
Detailed validation report with actionable items
- 참조: `src/types/graph/graph.ts`

---

## 4. Validation

### ConnectivityValidator
Validates connectivity and completeness of documentation across the codebase
- 참조: `src/validator/ConnectivityValidator.ts`

### ConventionValidator
Validates TSDoc comments against defined conventions

Enforces minimum documentation requirements based on symbol type: - Functions: summary, [object Object]param, [object Object]returns,  - Classes: summary, constructor docs, method docs,  - Interfaces: summary, property docs,  - Types: summary, usage description,  - Enums: summary, member docs,  - Variables: summary,  for exports
- 참조: `src/validator/ConventionValidator.ts`

### StrictModeValidator
Validator for strict mode documentation requirements
- 참조: `src/validator/StrictModeValidator.ts`

---

## 5. Fixers

### FixOptions
Options for fixing documentation
- 참조: `src/fixer/DocumentationFixer.ts`

### FixResult
Result of fixing a file
- 참조: `src/fixer/DocumentationFixer.ts`

### DocumentationFixer
Fixes documentation issues automatically
- 참조: `src/fixer/DocumentationFixer.ts`

### RecursiveImproveOptions
Options for recursive improvement
- 참조: `src/fixer/RecursiveImprover.ts`

### RecursiveImproveResult
Result of recursive improvement
- 참조: `src/fixer/RecursiveImprover.ts`

### RecursiveImprover
Recursively improves documentation quality
- 참조: `src/fixer/RecursiveImprover.ts`

---

## 6. Generators

### EnhancedMarkdownGenerator
Generator for enhanced markdown documentation
- 참조: `src/generator/EnhancedMarkdownGenerator.ts`

### InsightGenOptions
Options for insight document generation
- 참조: `src/generator/InsightDocGenerator.ts`

### InsightDocGenerator
Generates hierarchical insight documentation
- 참조: `src/generator/InsightDocGenerator.ts`

### MarkdownGenerator
Generates markdown documentation from parsed TSDoc comments
- 참조: `src/generator/MarkdownGenerator.ts`

### RelatedDocEntry
Related documentation entry
- 참조: `src/generator/RelatedDocsGenerator.ts`

### RelatedDocsGenerator
Generates markdown documentation showing a symbol and its related documentation
- 참조: `src/generator/RelatedDocsGenerator.ts`

---

## 7. Storage

### DatabaseManager
Database manager for symbol and documentation storage
- 참조: `src/storage/DatabaseManager.ts`

### SymbolRegistryManager
Symbol Registry Manager Manages the JSONL file containing ID-to-source mappings
- 참조: `src/storage/SymbolRegistryManager.ts`

---

## 8. Configuration

### ConfigManager
Configuration manager for tsdoc-edge
- 참조: `src/config/ConfigManager.ts`

### TsdocEdgeConfig
TSDoc Edge Configuration Types
- 참조: `src/types/config/config.ts`

### TsdocEdgeConfig
Main configuration interface for tsdoc-edge project
- 참조: `src/types/config/config.ts`

### ProjectConfig
Project metadata configuration
- 참조: `src/types/config/config.ts`

### PathsConfig
Storage paths configuration
- 참조: `src/types/config/config.ts`

### FoldConfig
Fold/unfold system configuration
- 참조: `src/types/config/config.ts`

### ValidationConfig
Validation rules configuration
- 참조: `src/types/config/config.ts`

### GeneratorConfig
Generator options configuration
- 참조: `src/types/config/config.ts`

---

## 9. Utilities

### CommentExporter
Exports TSDoc comments from TypeScript files to Markdown format
- 참조: `src/fold/CommentExporter.ts`

### CommentImporter
Imports comment states from Markdown and applies to TypeScript files
- 참조: `src/fold/CommentImporter.ts`

### CommentStateManager
Manages comment states across the project
- 참조: `src/fold/CommentStateManager.ts`

### ScannerConfig
Scanner configuration
- 참조: `src/scanner/FileScanner.ts`

### ScanResult
Scan result statistics
- 참조: `src/scanner/FileScanner.ts`

### FileScanner
File scanner for full codebase indexing
- 참조: `src/scanner/FileScanner.ts`

### DocQualityScore
Types for code analysis and health checking
- 참조: `src/types/analysis/quality.ts`

### DocQualityScore
Symbol documentation quality score
- 참조: `src/types/analysis/quality.ts`

### TestCoverageInfo
Test coverage information
- 참조: `src/types/analysis/quality.ts`

### CodeHealthMetrics
Code health metrics
- 참조: `src/types/analysis/quality.ts`

### ImprovementSuggestion
Improvement suggestion
- 참조: `src/types/analysis/quality.ts`

### AnalysisReport
Analysis report
- 참조: `src/types/analysis/quality.ts`

### AnalysisOptions
Analysis options
- 참조: `src/types/analysis/quality.ts`

### ParsedDocComment
Represents a parsed TSDoc comment with validation results
- 참조: `src/types/core/parse.ts`

### ValidationResult
Validation result for a single rule
- 참조: `src/types/core/parse.ts`

### DTOClassification
DTO classification result
- 참조: `src/types/domain/data-flow.ts`

### TransformationStep
Transformation step in a chain
- 참조: `src/types/domain/data-flow.ts`

### DataTransformationChain
Complete transformation chain
- 참조: `src/types/domain/data-flow.ts`

### TransformationPath
Transformation path between two types
- 참조: `src/types/domain/data-flow.ts`

### DataFlowAnalysisResult
Data flow analysis result
- 참조: `src/types/domain/data-flow.ts`

### InterfaceInfo
Interface-specific information
- 참조: `src/types/domain/interface.ts`

### InterfaceProperty
Interface property information
- 참조: `src/types/domain/interface.ts`

### InterfaceMethod
Interface method information
- 참조: `src/types/domain/interface.ts`

### MethodParameter
Method parameter information
- 참조: `src/types/domain/interface.ts`

### InterfaceDependency
Interface dependency information
- 참조: `src/types/domain/interface.ts`

### DomainStructure
Domain structure analysis result
- 참조: `src/types/domain/interface.ts`

### InterfaceDependencyGraph
Interface dependency graph
- 참조: `src/types/domain/interface.ts`

### InterfaceAnalysisOptions
Interface analysis options
- 참조: `src/types/domain/interface.ts`

### FeatureDocument
Feature document types
- 참조: `src/types/feature/feature.ts`

### FeatureDocument
Feature document metadata
- 참조: `src/types/feature/feature.ts`

### SymbolReference
Symbol reference in feature document Syntax: [object Object]symbolId[object Object] or [object Object]
- 참조: `src/types/feature/feature.ts`

### FeatureIndex
Feature document index
- 참조: `src/types/feature/feature.ts`

### SourceRef
Symbol registry types for JSONL storage
- 참조: `src/types/registry/registry.ts`

### SourceRef
Source reference pointing to code location
- 참조: `src/types/registry/registry.ts`

### DependencyRelation
Dependency relationship
- 참조: `src/types/registry/registry.ts`

### SymbolRegistryEntry
Symbol registry entry (stored in JSONL) This is the minimal data needed to map IDs to source locations
- 참조: `src/types/registry/registry.ts`

### SymbolRegistry
Symbol registry managing ID-to-source mappings
- 참조: `src/types/registry/registry.ts`

### CommentLocation
Location of a comment in source code
- 참조: `src/types/state/comment.ts`

### CommentState
State of a single comment
- 참조: `src/types/state/comment.ts`

### FileCommentState
State storage for a single file
- 참조: `src/types/state/comment.ts`

### StateStorage
Complete state storage for all files
- 참조: `src/types/state/comment.ts`

### CollapseOptions
Options for collapsing comments
- 참조: `src/types/state/comment.ts`

### ExpandOptions
Options for expanding comments
- 참조: `src/types/state/comment.ts`

### ExportResult
Result of export operation
- 참조: `src/types/state/comment.ts`

### ImportResult
Result of import operation
- 참조: `src/types/state/comment.ts`

### FileStatusSummary
Status summary for a file
- 참조: `src/types/state/comment.ts`

### ImportanceCriteria
Criteria for determining symbol importance
- 참조: `src/types/statistics.ts`

### DetectableStats
Statistics for detectable symbols
- 참조: `src/types/statistics.ts`

### ImportanceStats
Statistics grouped by importance level
- 참조: `src/types/statistics.ts`

### StatsDelta
Change delta between two statistics
- 참조: `src/types/statistics.ts`

### StatsComparison
Comparison between two statistics
- 참조: `src/types/statistics.ts`

### SymbolChange
Symbol documentation change
- 참조: `src/types/statistics.ts`

### TrackableStatistics
Trackable statistics snapshot
- 참조: `src/types/statistics.ts`

### StatsHistoryEntry
Statistics history entry
- 참조: `src/types/statistics.ts`

### StatsHistory
Statistics history storage
- 참조: `src/types/statistics.ts`

### WarningConditions
Warning conditions for statistics comparison
- 참조: `src/types/statistics.ts`

### StatsCollectionOptions
Statistics collection options
- 참조: `src/types/statistics.ts`

### SymbolRelationship
Represents a relationship between two symbols
- 참조: `src/types/tags/base.ts`

### ContractSpec
Represents a contract specification for a symbol
- 참조: `src/types/tags/base.ts`

### TestMapping
Represents a test mapping for a symbol
- 참조: `src/types/tags/base.ts`

### ResponsibilitySpec
Represents design responsibility of a symbol
- 참조: `src/types/tags/base.ts`

### DesignDecision
Represents a design decision record
- 참조: `src/types/tags/base.ts`

### ProblemSolving
Enhanced documentation tags for Strict Mode
- 참조: `src/types/tags/enhanced.ts`

### ProblemSolving
Problem solving documentation
- 참조: `src/types/tags/enhanced.ts`

### Functionality
Functionality documentation
- 참조: `src/types/tags/enhanced.ts`

### ErrorExperience
Error experience documentation
- 참조: `src/types/tags/enhanced.ts`

### DecisionRecord
Decision record
- 참조: `src/types/tags/enhanced.ts`

### DependencySpec
Dependency specification (enhanced)
- 참조: `src/types/tags/enhanced.ts`

### FuturePlan
Future plan documentation
- 참조: `src/types/tags/enhanced.ts`

### EnhancedSymbolDoc
Enhanced symbol documentation (Strict Mode) Requires all 6 categories to be present for public APIs
- 참조: `src/types/tags/enhanced.ts`

### StrictModeValidation
Strict mode validation result
- 참조: `src/types/tags/enhanced.ts`

### IdGeneratorOptions
ID Generator for symbol identification
- 참조: `src/utils/IdGenerator.ts`

### IdGeneratorOptions
ID generation options
- 참조: `src/utils/IdGenerator.ts`

### IdGenerator
ID Generator
- 참조: `src/utils/IdGenerator.ts`

---
