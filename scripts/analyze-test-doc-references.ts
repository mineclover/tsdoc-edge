...
function analyzeTestDocReferences(): TestDocAnalysis {
  ...
  const testSymbols = allSymbols.filter(s =>
    s.type === 'test-suite' || s.type === 'test-case' || s.type === 'test-scenario'
  );
  ...
  const docRefCoverage = testSymbols.length > 0
    ? (testsWithDocs.size / testSymbols.length) * 100
    : 0;
  ...
  if (topDocumented.length > 0) {
    ...
    topDocumented.forEach((item, idx) => {
      console.log(`  ${(idx + 1).toString().padStart(2)}. ${item.testSymbol.padEnd(50)} ${item.docReferences} refs`);
    });
  }
  ...
  const docsReferencingTests = db['db'].prepare(`
    SELECT COUNT(*) as count
    FROM unified_relationships
    WHERE type IN ('conceptual-relation', 'doc-reference')
