/**
 * Move Command - Move file to new directory and update all references
 *
 * @packageDocumentation
 * @responsibility Move documentation files to new directories and update all references
 * @contract Safely move files while maintaining reference integrity
 *
 * @problem Moving files manually breaks references (backlinks, paths, relative links)
 * @solves Automatically updates all references when moving files to new directories
 ...
 * Move Command
 *
 * Moves a documentation file to a new directory and updates all references to it
 *
 * @public
 * @responsibility Move files safely with reference updates
 * @contract Find all references, update them, move file, validate result
 *
 * @example
 * ```bash
 * # Move file to new directory
 * tsdoc-edge move features/old.md workflows/old.md
 *
 * # Move with new name
 * tsdoc-edge move features/old.md workflows/new.md
 *
 * # Dry run (preview only)
 * tsdoc-edge move old.md new-dir/old.md --dry-run
 *
 * # Auto-confirm
 * tsdoc-edge move old.md new-dir/old.md --yes
 * ```
 */
export class MoveCommand extends BaseCommand {
  ...
  getDescription(): string {
    return 'Move file to new directory and update all references';
  }

  protected getUsage(): string {
    return `tsdoc-edge move <old-path> <new-path> [options]
  ...
  Examples:
    tsdoc-edge move features/old.md workflows/old.md
    tsdoc-edge move old.md new-dir/new-name.md --dry-run
    tsdoc-edge move path/old.md new-path/old.md --yes`;
  }
  ...
  async execute(args: string[]): Promise<CommandResult> {
    return this.executeWithErrorHandling(async () => {
      ...
      // Step 1: Validate
      this.printInfo('Step 1: Validating move operation...');
      ...
      if (!dryRun && !autoConfirm) {
        console.log();
        const confirmed = await this.confirm('Proceed with move?');
        if (!confirmed) {
      ...
      console.log(`  ${colors.green}✓${colors.reset} Moved file`);
      ...
      // Step 6: Success
      console.log();
      this.printSuccess('Move complete!');
      console.log();
  ...
  private async validate(
    ...
    // Check if actually moving to different directory
    const oldDir = path.dirname(absoluteOldPath);
    if (oldDir === newDir && path.basename(absoluteOldPath) === path.basename(absoluteNewPath)) {
      warnings.push('Old and new paths are identical - no move needed');
    }
  ...
  private previewChanges(
    ...
    // Show file move
    console.log(`${colors.bold}File to move:${colors.reset}`);
