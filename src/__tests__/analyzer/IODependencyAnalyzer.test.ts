...
describe('IODependencyAnalyzer', () => {
  ...
  describe('detectPipelines', () => {
    it('should return empty array for empty dependencies', () => {
      const graph = createMockGraph([]);
      const analyzer = new IODependencyAnalyzer(graph);

      const pipelines = analyzer.detectPipelines([]);

      expect(pipelines).toHaveLength(0);
    });

    it('should detect A -> B -> C pipeline', () => {
      const graph = createMockGraph([]);
      const analyzer = new IODependencyAnalyzer(graph);

      const dependencies: UnifiedRelationship[] = [
        {
          id: 'io-dep-a-b',
          type: 'io-dependency',
          category: 'data-flow',
      ...
      const pipelines = analyzer.detectPipelines(dependencies);

      expect(pipelines.length).toBeGreaterThan(0);
      const pipeline = pipelines[0];
      expect(pipeline.type).toBe('pipeline');
      expect(pipeline.category).toBe('data-flow');
      expect(pipeline.properties?.chain).toEqual(['funcA', 'funcB', 'funcC']);
      expect(pipeline.properties?.length).toBe(3);
    });

    it('should detect multiple pipelines', () => {
      const graph = createMockGraph([]);
      const analyzer = new IODependencyAnalyzer(graph);
    ...
    it('should detect branching pipelines (A -> B -> C and A -> B -> D)', () => {
      const graph = createMockGraph([]);
      const analyzer = new IODependencyAnalyzer(graph);
      ...
      // Should detect both A -> B -> C and A -> B -> D
      expect(pipelines.length).toBe(2);
    });

    it('should create correct pipeline relationship structure', () => {
      const graph = createMockGraph([]);
      const analyzer = new IODependencyAnalyzer(graph);
      ...
      expect(pipeline.id).toBe('pipeline-step1-step2-step3');
      expect(pipeline.type).toBe('pipeline');
      expect(pipeline.category).toBe('data-flow');
      expect(pipeline.from).toEqual(['step1', 'step2']);
      expect(pipeline.to).toBe('step3');
      expect(pipeline.direction).toBe('unidirectional');
      expect(pipeline.strength).toBe('medium');
      expect(pipeline.confidence).toBe(0.8);
      expect(pipeline.discoveredBy).toBe('static-analysis');
      expect(pipeline.evidence[0].context).toContain('step1');
      expect(pipeline.evidence[0].context).toContain('step2');
      expect(pipeline.evidence[0].context).toContain('step3');
      expect(pipeline.description).toContain('Pipeline');
    });

    it('should handle non-string from/to values gracefully', () => {
      const graph = createMockGraph([]);
      const analyzer = new IODependencyAnalyzer(graph);
      ...
      const dependencies: UnifiedRelationship[] = [
        {
          ...
          type: 'io-dependency',
          category: 'data-flow',
          from: ['multiple', 'sources'], // Array instead of string
          to: 'target',
          direction: 'unidirectional',
          strength: 'medium',
          evidence: [],
